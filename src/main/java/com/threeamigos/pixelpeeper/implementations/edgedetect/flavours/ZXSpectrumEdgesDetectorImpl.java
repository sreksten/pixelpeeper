...
public class ZXSpectrumEdgesDetectorImpl implements ZXSpectrumEdgesDetector {
    ...
    public ZXSpectrumEdgesDetectorImpl(ZXSpectrumEdgesDetectorPreferences zxSpectrumEdgesDetectorPreferences,
    ...
    private void mix(HSL[] originalPalette, List<DitheredHSL> destinationList) {
        ...
        if (zxSpectrumEdgesDetectorPreferences.isSkinTonesMappingEnabled()) {
    ...
    private void processBlock(BufferedImage newImage, int startX, int startY, int endX, int endY) {
        ...
        if (zxSpectrumEdgesDetectorPreferences.isColorClashEnabled()) {
            // Count color usage
    ...
    private DitheredHSL mapToClosestPaletteColor(RGB rgb, HSL hsl, List<DitheredHSL> ditheredPalette) {
        ...
        if (saturation < zxSpectrumEdgesDetectorPreferences.getSaturationThreshold()) {
            return getBlackAndWhiteDitheredHSL(ditheredPalette, lightness);
        }
        if (lightness < zxSpectrumEdgesDetectorPreferences.getLightnessMinThreshold()) return ditheredPalette.get(PALETTE_BLACK_INDEX);
        if (lightness > zxSpectrumEdgesDetectorPreferences.getLightnessMaxThreshold()) return ditheredPalette.get(PALETTE_WHITE_INDEX);
    ...
    private double calculateColorDistance(
        ...
        final double HUE_WEIGHT = zxSpectrumEdgesDetectorPreferences.getHueWeight();
        final double SATURATION_WEIGHT = zxSpectrumEdgesDetectorPreferences.getSaturationWeight();
        final double LIGHTNESS_WEIGHT = zxSpectrumEdgesDetectorPreferences.getLightnessWeight();

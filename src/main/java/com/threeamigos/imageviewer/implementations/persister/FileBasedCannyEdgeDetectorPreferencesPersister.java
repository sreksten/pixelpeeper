...
public class FileBasedCannyEdgeDetectorPreferencesPersister extends FileBasedAbstractPreferencesPersister<CannyEdgeDetectorPreferences> implements Persister<CannyEdgeDetectorPreferences> {
	...
	private void checkBoundaries(float lowThreshold, float highThreshold, float gaussianKernelRadius,
		...
		if (lowThreshold < 0) {
			throw new IllegalArgumentException("Invalid Canny Edges Detector preferences: low threshold must be greater or equal than 0");
		}
		if (highThreshold < 0) {
			throw new IllegalArgumentException("Invalid Canny Edges Detector preferences: high threshold must be greater or equal than 0");
		}
		if (gaussianKernelRadius < 0.1f) {
			throw new IllegalArgumentException("Invalid Canny Edges Detector preferences: gaussian kernel radius must be greater or equal than 0.1");
		}
		if (gaussianKernelWidth < 2) {
			throw new IllegalArgumentException("Invalid Canny Edges Detector preferences: gaussian kernel width must be greater or equal than 2");
		}
	}

	@Override
	protected void saveImpl(PrintWriter writer, CannyEdgeDetectorPreferences cannyEdgesDetectorPreferences) throws IOException {
